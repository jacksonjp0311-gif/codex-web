...
def resonance_phi_direct(E, I, C, psi, eps=1e-12):
    ...
    varp = float(np.var(p))
    denom = (varp + eps)
    phase_var = float(np.var(np.angle(psi[p > eps]))) if np.any(
        p > eps) else 0.0
...
def N(vec):
        ...
        return -1j * alpha_total * Phi_loc * \
            vec, (E_loc, I_loc, C_loc, Phi_loc)
        N1, m1 = N(psi)
        ...
        c = expL(dt, psi + dt * N3)
        N4, m4 = N(c)
        psi_next = expL(dt, psi) + (dt / 6.0) * (expL(dt, N1) +
                        2 * expL(dt, N2) + 2 * expL(dt, N3) + expL(dt, N4))
...
def adapt_alphas_PID(
    ...
    alpha_E_sm = float(
        max(cfg["alpha_min"], min(cfg["alpha_max"], alpha_E_sm)))
    alpha_I_sm = float(
        max(cfg["alpha_min"], min(cfg["alpha_max"], alpha_I_sm)))
    alpha_C_sm = float(
        max(cfg["alpha_min"], min(cfg["alpha_max"], alpha_C_sm)))
...
def run_single(seed: int, run_id: int, save_png=True):
    ...
    psi[N // 2] = 1.0 + 0j
    psi *= np.exp(1j * np.random.uniform(-cfg["initial_phase_jitter"],
                  cfg["initial_phase_jitter"], size=psi.shape))
    ...
    for step_idx in range(cfg["timesteps"]):
        if cfg["max_runtime_seconds"] and (
    time.time() - start_time) > cfg["max_runtime_seconds"]:
            break
    Wt_val = cfg["W_base"] + cfg["W_amp"] * \
        math.sin(2.0 * math.pi * (t / cfg["W_period"]))
    H = build_H_dense(cfg["t_hop"], cfg["t_z"], Wt_val)
    psi, (E_n, I_n, C_n, Phi_n) = if_rk4_step(psi, H, dt_local, alpha_total)
    T.append(t); Wt.append(Wt_val); E_t.append(E_n); I_t.append(
        I_n); C_t.append(C_n); Phi_t.append(Phi_n)
    aE_t.append(alpha_E); aI_t.append(alpha_I); aC_t.append(
        alpha_C); aTot_t.append(alpha_total)
    ...
    alpha_E, alpha_I, alpha_C = adapt_alphas_PID(
    alpha_E, alpha_I, alpha_C, Phi_recent, C_recent, pid_state)
    alpha_total = combine_alpha(alpha_E, alpha_I, alpha_C)
    ...
    gap = float(np.mean(np.diff(np.sort(np.real(la.eigvalsh(H)))))
                ) if 'H' in locals() else 0.0
