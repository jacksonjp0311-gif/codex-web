...
- **What:** `codex/state` is a Codex runtime domain folder that stores module logic, state, logs, or related artifacts for the **state** subsystem.
- **Why:** This folder exists to isolate `state` responsibilities, reduce coupling with other domains, and make evolution/versioning safer over time.
...
- **Where:** Path: `codex/state` (within the core Codex runtime tree).
