...
- **What:** `codex/memory` is a Codex runtime domain folder that stores module logic, state, logs, or related artifacts for the **memory** subsystem.
- **Why:** This folder exists to isolate `memory` responsibilities, reduce coupling with other domains, and make evolution/versioning safer over time.
...
- **Where:** Path: `codex/memory` (within the core Codex runtime tree).
