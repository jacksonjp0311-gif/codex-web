...
# -*- coding: utf-8 -*-
# ╔══════════════════════════════════════════════════════════════╗
# ║  QIM v6.2 — AFM SUPER-RESOLUTION ENGINE (12×)                ║
...
def enhance_field(field, up_factor: int = 12):
    """
    Codex QIM v6.2 sharpener (GEO v1.0 aligned):
...
def make_superres_visuals(dphi: np.ndarray,
    ...
    plt.title("QIM v6.2 AFM Δφ central slice (12× super-res)")
    plt.colorbar()
    ...
    plt.title("QIM v6.2 AFM Δφ max projection (12× super-res)")
    plt.colorbar()
    ...
    plt.title("QIM v6.2 AFM Ω max projection (GEO v1.0, 12×)")
    plt.colorbar()
    ...
    plt.title("QIM v6.2 AFM curvature proxy (Ω Laplace, 12×)")
    plt.colorbar()
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(
        description="QIM v6.2 — AFM Super-Resolution Engine (12×)"
    )
    ...
    log("QIM v6.2 · AFM Super-Resolution Engine (12×) starting…")
    ...
    try:
        ...
        log("QIM v6.2 run complete.")
        return 0

    except Exception as e:
        err_msg = f"QIM v6.2 encountered an error: {repr(e)}"
        log(err_msg)
