#!/usr/bin/env python3
# ╔══════════════════════════════════════════════════════════════╗
# ║  QIM v6.5.3 — AFM SUPER-RES ENGINE (SAFE ZOOM)               ║
# ║  AFM-bound 4D Δφ field with GEO v1.0 + H16B fractal geometry ║
# ╚══════════════════════════════════════════════════════════════╝
...
def load_afm(path: Path):
    ...
    if arr.ndim == 2:
        ...
        # take central volume if an extra dim is present
...
def super_resolve(vol: np.ndarray, factor: int, max_size: int = 128):
    """
    SAFE super-resolution:
    • Uses scipy.ndimage.zoom (continuous interpolation)
    • Caps final size at max_size in each dimension
    to avoid 27GB meshgrids.
    """
    nx, ny, nz = vol.shape
...
def fractal_dim(vol: np.ndarray):
    """
    Simple 3D fractal dimension proxy:
    slice-based box-counting across scales.
    """
...
def main(root, state_d, vis_d, ledger_d, logs_d, afm_path, superres):
    ...
    log_path = logs_dir / f"qim_v6_5_3_run_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.log"
    with log_path.open("w", encoding="utf-8") as lf:
        ...
        log("QIM v6.5.3 — AFM Super-Res Engine starting…")
        ...
        try:
            ...
            p1 = visuals / "qim_v6_5_3_dphi_central.png"
            write_img(p1, dphi_c, "QIM v6.5.3 Δφ central slice")
            vis["dphi_central"] = str(p1)

            p2 = visuals / "qim_v6_5_3_dphi_maxproj.png"
            write_img(p2, maxp, "QIM v6.5.3 Δφ max projection")
            vis["dphi_maxproj"] = str(p2)

            p3 = visuals / "qim_v6_5_3_omega_maxproj.png"
            write_img(p3, omega_max, "QIM v6.5.3 Ω max projection")
            ...
            plt.xlabel("t")
            plt.ylabel("<|V|>")
            plt.title("QIM v6.5.3 AFM Super-Res resonance curve")
            p4 = visuals / "qim_v6_5_3_resonance_curve.png"
            plt.savefig(p4, bbox_inches="tight")
            plt.close()
            vis["resonance_curve"] = str(p4)

            ts_tag = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
            state_path = state_dir / f"qim_v6_5_3_state_{ts_tag}.json"
            ...
            ledger_path = ledger_dir / "qim_v6_5_3_ledger.jsonl"
            row = {
                "timestamp": now_iso(),
                "mode": "afm-superres",
            ...
            log("QIM v6.5.3 AFM Super-Res run complete.")

        except Exception as e:
            err = "QIM v6.5.3 encountered an error: " + repr(e)
