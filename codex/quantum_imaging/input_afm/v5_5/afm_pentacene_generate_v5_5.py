...
• Adds tip-convolution blur + mild noise
• Normalizes to [0,1] and saves as .npy (AFM-like force field)
"""
...
def make_pentacene_afm(shape=(64, 64, 64), rings=5, noise_level=0.02):
    ...
    # Pentacene = 5 fused benzene-like rings along x-axis
    # Place 5 ring centers, slightly staggered to avoid perfect symmetry
    ...
    for cx, cy, cz in zip(xs, ys, zs):
        ...
        R_z  = np.abs(Z - cz)
        # Ring ridge in XY, softened along Z
        ring = np.exp(-((R_xy - 0.55) ** 2) * 28.0) * np.exp(-R_z * 10.0)
        base += ring.astype(np.float32)

    # Add central backbone density
    backbone = np.exp(-((X) ** 2) * 6.0) * np.exp(-(Y ** 2) * 10.0)
    base += 0.5 * backbone.astype(np.float32)

    # Approximate tip-convolution via small Gaussian blur
    # (simple separable smoothing with three 1D kernels)
    def gaussian_kernel_1d(sigma, radius=3):
    ...
    # Invert contrast to mimic AFM height (bright = high tip, low = well)
    vol = vol.max() - vol
...
def main():
    ...
    vol = make_pentacene_afm(shape=(64, 64, 64), rings=5, noise_level=0.02)
