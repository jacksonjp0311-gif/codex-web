#!/usr/bin/env python3
# ╔══════════════════════════════════════════════════════════════╗
# ║  QIM v5.0 — CROSS-MODULE INSIGHT ENGINE (DOMINANT MODE)      ║
# ║  Codex ΔΦ Perception Kernel (5-Channel, Multi-Scale)         ║
# ╚══════════════════════════════════════════════════════════════╝
...
def synthetic_volume(shape=(64, 64, 64), seed=51):
    np.random.seed(seed)
    nx, ny, nz = shape
...
def dominant_fusion(channels, metrics_map, log_fp=None):
    ...
    # α based on teacher curvature: more curvature → stronger teaching
...
def make_visuals(V_unified, dphi_unified, omega_unified, out_dir: Path, prefix: str):
    ...
    plt.title("QIM v5.0 unified dphi central slice")
    plt.colorbar()
    ...
    plt.title("QIM v5.0 unified dphi max projection")
    plt.colorbar()
    ...
    plt.title("QIM v5.0 unified omega max projection")
    plt.colorbar()
    ...
    plt.title("QIM v5.0 cross-module resonance curve")
...
def write_state_ledger_spec(root_dir: Path,
    ...
    state_path = state_dir / f"qim_v5_0_state_{ts_tag}.json"
    ledger_path = ledger_dir / "qim_v5_0_ledger.jsonl"
    spec_path = (root_dir / "codex" / "quantum_imaging" / "engine" / "codex_qim_v5_1_autogen_spec.json")
    ...
    state_obj = {
        ...
        "codex": {
            ...
            "memory": {
                "node": "QIM",
                "baseline_version": "4.9",
                "current_version": "5.0",
                "mode": "cross-module-dominant",
            },
        },
        "visuals": visuals,
    }
    ...
    spec_obj = {
        ...
        "recommendation": {
            ...
            "recommended_T": int(T),
            "next_focus": "stabilize dominant-teacher dynamics, ingest real Solar/QCX/ThirdEye fields, and tune Ω-curvature across channels",
        },
    }
...
def main():
    ...
    input_dir = Path(args.input_afm_dir) if args.input_afm_dir else (root_dir / "codex" / "quantum_imaging" / "input_afm" / "v5_0")

    log_fp = None
    try:
        if logs_dir is not None:
            logs_dir.mkdir(parents=True, exist_ok=True)
            log_path = logs_dir / f"qim_v5_0_run_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.log"
            log_fp = log_path.open("w", encoding="utf-8")
    except Exception:
        log_fp = None

    log(log_fp, "QIM v5.0 — Cross-Module Insight Engine (Dominant Mode) starting…")
    ...
    try:
        ...
        base3d = synthetic_volume(shape=(64, 64, 64), seed=131)
        T = 40
        ...
        visuals = make_visuals(V_unified, dphi_unified, omega_unified, visuals_dir, "qim_v5_0_field")
        ...
        log(log_fp, f"v5.1 autogen spec → {spec_path}")
        log(log_fp, "QIM v5.0 run complete.")

    except Exception as e:
        err = "QIM v5.0 encountered an error: " + repr(e)
