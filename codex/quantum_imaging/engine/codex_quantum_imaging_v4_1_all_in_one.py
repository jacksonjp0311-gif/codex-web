#!/usr/bin/env python3
# QIM v4.1 — Field Convergence Engine (All-In-One)
# Role:
#   • Load or synthesize 3D AFM-like volume
#   • Extend to 4D (x, y, z, t) evolving field
#   • Compute ΔΦ gradients, global ΔΦ (H₁₉), E–I–C triad, cusp metrics
...
# ─────────────────────────────────────────────
# Utility: safe float conversion for JSON
# ─────────────────────────────────────────────
def f(x):
    try:
        return float(x)
...
def synthetic_volume(shape=(64, 64, 64), seed=19):
    ...
    # Radial shell + interference rings (Codex QIM style)
...
def load_afm_stack(input_dir: Path, shape=(64, 64, 64)):
    """
    Future: load real AFM PNG stack.
    For now: if any PNG exists, we still synthesize but mark synthetic=False.
    """
    pngs = sorted(input_dir.glob("*.png"))
    if len(pngs) == 0:
        vol = synthetic_volume(shape=shape)
        return vol, True, 0
    # In v4.1 we treat presence of PNGs as "real context" but still use
    # synthetic generator (to avoid dependency breakage across machines).
    vol = synthetic_volume(shape=shape)
...
def build_4d_field(volume3d, T=40):
    ...
    Simple oscillatory evolution around base volume.
    """
    nx, ny, nz = volume3d.shape
    V = np.zeros((T, nx, ny, nz), dtype=np.float32)

    # Precompute radius for phase modulation
...
# ─────────────────────────────────────────────
# 3) ΔΦ, TRIAD, H₁₉ & CUSP METRICS
# ─────────────────────────────────────────────
def compute_delta_phi_4d(V):
    # central differences across space only (x,y,z)
    T, nx, ny, nz = V.shape
...
def compute_global_metrics(V, dphi):
    ...
    # I = mean |∇V| = mean ΔΦ
    I = f(np.mean(dphi))
    # H₁₉: global ΔΦ field integration
    delta_phi_global = f(np.mean(dphi))

    # Cusp-inspired λ_eff (normalized ΔΦ global)
    # Keep in [0, 0.99) to avoid singularity at 1
    lam_eff = min(0.99, delta_phi_global / (1.0 + delta_phi_global))
    # Barrier scaling ~ (1-λ)^(3/2) * (EI)^(3/2)
...
def compute_harmonics(dphi):
    """
    Core / shell / void roles ala Codex QIM convention,
    but using 4D ΔΦ distribution.
    """
    vals = dphi.flatten()
...
def save_visuals(V, dphi, visuals_dir: Path, prefix: str):
    ...
    plt.title("QIM v4.1 ΔΦ central slice (t_mid, z_mid)")
    plt.colorbar()
    out_central = visuals_dir / f"{prefix}_delta_phi_central.png"
    fig.savefig(out_central, bbox_inches="tight")
    plt.close(fig)
    paths["delta_phi_central"] = str(out_central)

    # Max projection over t and z
    maxproj = dphi.max(axis=0).max(axis=2)
    fig = plt.figure()
    plt.imshow(maxproj, origin="lower")
    plt.title("QIM v4.1 ΔΦ max projection (t,z)")
    plt.colorbar()
    out_max = visuals_dir / f"{prefix}_delta_phi_maxproj.png"
    fig.savefig(out_max, bbox_inches="tight")
    plt.close(fig)
    paths["delta_phi_maxproj"] = str(out_max)

    # Horizon-style projection: max over t and x → (y,z)
    horizon = dphi.max(axis=0).max(axis=0)
    fig = plt.figure()
    plt.imshow(horizon, origin="lower")
    plt.title("QIM v4.1 Horizon max projection (t,x)")
    plt.colorbar()
    ...
    # Resonance curve: mean field energy vs t
    energy_t = np.mean(np.abs(V), axis=(1, 2, 3))
    fig = plt.figure()
    plt.plot(range(T), energy_t)
    plt.xlabel("t (frame)")
    plt.ylabel("⟨|V|⟩")
    plt.title("QIM v4.1 Resonance curve (mean |V| vs t)")
    ...
    # Optional GIF of ΔΦ central slice over time
    if IMAGEIO_OK:
        frames = []
        for t in range(T):
            sl = dphi[t, :, :, z_mid]
        ...
        try:
            ...
            # GIF creation failed; ignore, keep other visuals
            pass
...
def write_state_and_ledger(state_dir: Path,
    ...
    state_path = state_dir / f"qim_v4_1_field_state_{ts}.json"
    ledger_path = ledger_dir / "qim_v4_1_ledger.jsonl"

    T, nx, ny, nz = V.shape

    state_obj = {
        "protocol": "CodexQIMFieldConvergence",
        "version": "4.1",
        ...
        "codex": {
            "H_layer": {
                "H7": 0.70,
                "H19": "Global ΔΦ integration layer (4D field → C_effective)"
            },
            "laws": {
            ...
            "tesseract_alignment": {
                "note": "Field is prepared for glyph-compressed mapping (ls/ic/ul pairs)",
                "modules": {
                    "QIM": "𓇳QIM",
                    "QCX": "𓂀QCX",
                    "GIZA": "𓊹GIZA",
    ...
    ledger_obj = {
        "timestamp": datetime.utcnow().isoformat() + "Z",
        "mode": "qim-v4-1-field-convergence",
...
def main():
    ...
    if logs_dir is not None:
        ...
        log_file = logs_dir / f"qim_v4_1_run_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}.log"
    else:
        log_file = None
    ...
    t0 = datetime.utcnow()
    log("QIM v4.1 — Field Convergence Engine (Python) starting…")
    ...
    try:
        ...
        # 3) ΔΦ, metrics, harmonics
        dphi = compute_delta_phi_4d(V)
        log("Computed ΔΦ field over 4D volume.")

        metrics = compute_global_metrics(V, dphi)
        harmonics = compute_harmonics(dphi)
        log(f"Global triad: {metrics.get('triad',{})}")
        log(f"H₁₉ ΔΦ_global: {metrics.get('delta_phi_global',0.0)}")
        log(f"Cusp λ_eff: {metrics.get('lambda_eff',0.0)}, barrier_scale: {metrics.get('barrier_scale',0.0)}")
        log(f"Harmonics: {harmonics}")

        # 4) Visuals
        ts_tag = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
        prefix = f"qim_v4_1_field_{ts_tag}"
        ...
        log(f"QIM v4.1 run complete. Runtime: {dt:.3f} s")

    except Exception as e:
        err_msg = "QIM v4.1 encountered an error: " + repr(e)
