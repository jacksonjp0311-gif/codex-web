#!/usr/bin/env python3
# ╔══════════════════════════════════════════════════════════════╗
# ║  QIM v5.3 — REAL-CHANNEL VISUAL EVOLUTION (INSIGHT ENGINE)   ║
# ║  AFM-bound ΔΦ geometry + multi-scale visuals (5-channel)     ║
# ╚══════════════════════════════════════════════════════════════╝
...
def load_real_afm_cube(input_dir: Path, target_shape=(64, 64, 64), log_fp=None):
    ...
    try:
        ...
        if arr.ndim == 2:
            # Expand to 3D (simple stack)
            arr = np.stack([arr] * target_shape[2], axis=-1)
        elif arr.ndim == 4:
            # Take middle time slice if 4D
            arr = arr[arr.shape[0] // 2]

        # Simple center-crop / pad to match target_shape
...
def synthetic_volume(shape=(64, 64, 64), seed=153):
    np.random.seed(seed)
    nx, ny, nz = shape
...
# ╭──────────────────────────────────────────────────────────────╮
# │  2. CHANNEL SYNTHESIS (5 MODULES, REAL-BOUND)               │
# ╰──────────────────────────────────────────────────────────────╯

def synthesize_channels(base_afm_or_synth, T=40, log_fp=None):
    base3d = base_afm_or_synth

    # QIM: AFM-bound reference
    ...
    # AFM channel: horizon emphasis (reuse base)
    V_afm = build_4d_field(base3d * (1.0 + 0.4 * (base3d > np.median(base3d))),
                           T=T, phase_shift=2.8, radial_mod=1.0)

    log(log_fp, "[Base] Channels synthesized in AFM-bound geometry (or synthetic fallback).")
...
def dominant_fusion(channels, metrics_map, log_fp=None):
    ...
    # map curvature (~0.003–0.02) into [0.18, 0.42] as in v5.2 (visual evolution)
...
def make_visuals(V_unified, dphi_unified, omega_unified, out_dir: Path, prefix: str):
    ...
    plt.title("QIM v5.3 unified dphi central slice (real-channel ring lens)")
    plt.colorbar()
    ...
    plt.title("QIM v5.3 unified dphi max projection (3-node pattern)")
    plt.colorbar()
    ...
    plt.title("QIM v5.3 unified omega max projection (GEO v1.0)")
    plt.colorbar()
    ...
    plt.title("QIM v5.3 real-channel resonance curve")
    ...
    plt.ylabel("⟨Δφ⟩(r)")
    plt.title("QIM v5.3 radial Δφ profile (AFM-bound ring)")
    ...
    plt.xlabel("Δφ")
    plt.ylabel("count")
    plt.title("QIM v5.3 Δφ histogram (roughness distribution)")
    ...
    plt.xlabel("radius")
    plt.ylabel("t")
    plt.title("QIM v5.3 time–radius strip (real-channel breathing ring)")
...
def write_state_ledger_spec(root_dir: Path,
    ...
    state_path = state_dir / f"qim_v5_3_state_{ts_tag}.json"
    ledger_path = ledger_dir / "qim_v5_3_ledger.jsonl"
    spec_path = (root_dir / "codex" / "quantum_imaging" / "engine" / "codex_qim_v5_4_autogen_spec.json")
    ...
    state_obj = {
        "protocol": "CodexQIMRealChannelVisualEvolution",
        "version": "5.3",
        "timestamp": now_utc_iso(),
        "mode": "real-channel-visual-evolution",
        ...
        "codex": {
            "H_layers": {
                "H7": 0.70,
                "H16": "Insight / pattern geometry (C_geo, cross-module, AFM-bound)",
    ...
    spec_obj = {
        ...
        "recommendation": {
            ...
            "recommended_T": int(T),
            "next_focus": "stabilize AFM-bound teacher dynamics across QIM/Solar/QCX/ThirdEye and compare real vs synthetic time–radius signatures.",
        },
    }
...
def main():
    ...
    input_dir = Path(args.input_afm_dir) if args.input_afm_dir else (root_dir / "codex" / "quantum_imaging" / "input_afm" / "v5_3")
    ...
    log(log_fp, "QIM v5.3 — Real-Channel Visual Evolution Engine starting…")
    ...
    try:
        # Try binding to real AFM first
        afm_base = load_real_afm_cube(input_dir, target_shape=(64, 64, 64), log_fp=log_fp)
        if afm_base is not None:
            afm_mode = "real-afm-bound"
            base3d = afm_base
            log(log_fp, "[Mode] REAL AFM binding active.")
        else:
            afm_mode = "synthetic-fallback"
            base3d = synthetic_volume(shape=(64, 64, 64), seed=153)
            log(log_fp, "[Mode] No AFM detected or binding failed → synthetic AFM-style volume.")
        ...
        visuals = make_visuals(V_unified, dphi_unified, omega_unified, visuals_dir, "qim_v5_3_field")
        ...
        log(log_fp, f"v5.4 autogen spec → {spec_path}")
        log(log_fp, "QIM v5.3 run complete.")

    except Exception as e:
        err = "QIM v5.3 encountered an error: " + repr(e)
