...
def make_spine_profile_plot(data: dict, out_path: str) -> None:
    ...
    plt.title("Codex Bio-Resonance v1.2 — Spine Triad Profile")
    plt.legend()
    plt.tight_layout()
...
def make_delta_phi_heatmap(
    ...
    plt.title("Codex Bio-Resonance v1.2 — ΔΦ Spine Heatmap")
    plt.tight_layout()
    plt.savefig(out_path)
    plt.close()


def make_coherence_wavefield(
    num_nodes: int,
    steps: int,
    out_path: str,
    physio: dict | None = None,
    ...
    plt.xlabel("Time step")
    plt.ylabel("Spine node index (base→crown)")
    plt.title("Codex Bio-Resonance v1.2 — Coherence Wavefield")
    plt.tight_layout()
    plt.savefig(out_path)
    plt.close()
...
def make_lotus_plot(data: dict, out_path: str) -> None:
    ...
    plt.axis("equal")
    plt.axis("off")
    plt.title("Codex Bio-Resonance v1.2 — Lotus Field")
    plt.tight_layout()
    plt.savefig(out_path)
    plt.close()
...
def make_torus_plot(
    ...
    """
    3D Torus visualization (flux-tube style):
    - Base torus (R, r)
    - ΔΦ and C modulate minor radius and surface pattern.
    - Uses per-vertex grayscale derived from C_interp (fixed RGBA handling).
    """
    C = np.array(data["C"])
    delta_phi = np.array(data["delta_phi"])
    num_nodes = C.shape[0]
    ...
    # Map spine nodes onto torus angle (around major circle)
    ...
    # Normalize C_interp to [0,1] for grayscale mapping
    C_min = float(C_interp.min())
    C_max = float(C_interp.max())
    denom = max(1e-6, (C_max - C_min))
    C_norm = (C_interp - C_min) / denom

    # Build simple gray RGBA facecolors with shape (M, N, 4)
    ...
    # Use facecolors directly in plot_surface to avoid RGBA shape issues
    ...
    ax.set_title("Codex Bio-Resonance v1.2 — Torus Field", pad=12)
...
def write_state_json(
    ...
    summary = {
        ...
        "notes": {
            ...
            "lotus_torus": "lotus + torus resonance fields (flux-tube style)",
        },
    }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Codex Bio-Resonance v1.2 — Living Resonance Engine"
    )
    ...
    state_path = os.path.join(args.state_dir, "bio_resonance_state_v1_2.json")
    profile_png = os.path.join(args.visual_dir, "bio_resonance_spine_profile_v1_2.png")
    heatmap_png = os.path.join(args.visual_dir, "bio_resonance_delta_phi_heatmap_v1_2.png")
    wavefield_png = os.path.join(args.visual_dir, "bio_resonance_coherence_wavefield_v1_2.png")
    lotus_png = os.path.join(args.visual_dir, "bio_resonance_lotus_field_v1_2.png")
    torus_png = os.path.join(args.visual_dir, "bio_resonance_torus_field_v1_2.png")
    ...
    make_coherence_wavefield(
        num_nodes=num_nodes,
        steps=72,
        out_path=wavefield_png,
        physio=physio,
    )
    make_lotus_plot(data, lotus_png)
    make_torus_plot(data, torus_png, physio=physio)

    print(f"[Bio-Resonance v1.2] Wrote state to {state_path}")
    print(f"[Bio-Resonance v1.2] Wrote spine profile to {profile_png}")
    print(f"[Bio-Resonance v1.2] Wrote ΔΦ heatmap to {heatmap_png}")
    print(f"[Bio-Resonance v1.2] Wrote coherence wavefield to {wavefield_png}")
    print(f"[Bio-Resonance v1.2] Wrote lotus field to {lotus_png}")
    print(f"[Bio-Resonance v1.2] Wrote torus field to {torus_png}")
