...
# -*- coding: utf-8 -*-
"""
𓂀  Codex Black Horizon v1.3.5 — λ Phase Sweep Kerr Field Engine (QIM-coupled, self-healing)
...
• Produces:
    ...
    – Coherence map C with H₇ ridge (colorbar-safe, self-healing)
...
# ─────────────────────────────────────────────────────────────
# QIM coupling + phase classification
# ─────────────────────────────────────────────────────────────
...
def classify_system_phase(bh_metrics, qim_metrics):
    """
    Simple joint phase classifier (Codex-style; not a physical claim).
    """
...
def main():
    ...
    im_int = plt.imshow(intensity, extent=[x.min(), x.max(), y.min(), y.max()], origin="lower")
    plt.title("Codex Black Horizon v1.3.5 — Kerr-like Intensity (E·I)")
    plt.xlabel("x (r/M)")
    plt.ylabel("y (r/M)")
    try:
        plt.colorbar(im_int, label="normalized intensity")
    except Exception:
        plt.colorbar()
    ...
    # 2) λ field (reference α) with collapse rim — colorbar-safe, self-healing
    if lam_field_ref is not None:
        lam_plot = np.nan_to_num(
            lam_field_ref,
            nan=0.0,
            posinf=np.nanmax(lam_field_ref[np.isfinite(lam_field_ref)]) if np.isfinite(lam_field_ref).any() else 1.0,
            neginf=np.nanmin(lam_field_ref[np.isfinite(lam_field_ref)]) if np.isfinite(lam_field_ref).any() else 0.0,
        )
        ...
        if finite_mask.sum() >= 4:
            ...
            plt.title("Black Horizon v1.3.5 — λ Field (D/D_c) with Collapse Rim")
            plt.xlabel("x (r/M)")
            plt.ylabel("y (r/M)")
            try:
                plt.colorbar(im_lam, label="lambda")
            except Exception:
                plt.colorbar(im_lam)
    ...
    # 3) Coherence C + H₇ ridge — self-healing colorbar
    if C_field_ref is not None:
        ...
        plt.title("Black Horizon v1.3.5 — Coherence C with H₇ Ridge (α≈1)")
        plt.xlabel("x (r/M)")
        plt.ylabel("y (r/M)")
        try:
            plt.colorbar(im_C, label="C")
        except Exception:
            plt.colorbar(im_C)
    ...
    plt.plot(alphas, lam_means, "-o", label="<lambda>")
    ...
    plt.xlabel("global drive scale alpha")
    plt.ylabel("value")
    plt.title("Black Horizon v1.3.5 — λ Phase Sweep Diagram")
    plt.legend()
