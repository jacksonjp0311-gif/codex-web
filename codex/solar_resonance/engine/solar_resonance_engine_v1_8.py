...
state_dir   = module_root / "state" / "v1_8"
visuals_dir = module_root / "visuals" / "v1_8"
...
# ─────────────────────────────────────────
# Codex Solar Resonance Model v1.8
#   Solar QIM Temporal Oracle:
#   C, Ω, spectral ΔΦ, trend, QIM lattice (multi-scale) + SRS
# ─────────────────────────────────────────
def compute_state():
    import numpy as np
    ...
    # Combined coherence score (as in v1.6 / v1.7)
    coherence_score = 0.0
    try:
    ...
    # Temporal QIM note: N and harmonic_entropy encode "texture"
    Temporal = {
        "N_samples": N,
        "coherence_drift": dC,
        "trend_score": trend_score,
        "texture_entropy": harmonic_entropy,
    }
...
# ─────────────────────────────────────────
# Visuals (Solar QIM Temporal Oracle)
# ─────────────────────────────────────────
def make_visuals(state, t_idx, flux, bz, g_flux, g_bz, dphi_local, ts_tag):
    ...
    fig1.tight_layout()
    out1 = visuals_dir / f"solar_resonance_flux_bz_v1_8_{ts_tag}.png"
    fig1.savefig(out1)
    plt.close(fig1)
    ...
    fig2.tight_layout()
    out2 = visuals_dir / f"solar_resonance_grad_heatmap_v1_8_{ts_tag}.png"
    fig2.savefig(out2)
    plt.close(fig2)
    ...
    fig3.tight_layout()
    out3 = visuals_dir / f"solar_resonance_dphi_strip_v1_8_{ts_tag}.png"
    fig3.savefig(out3)
    plt.close(fig3)
    ...
    ax4.set_title("Solar ΔΦ Harmonic Spectrum v1.8")
    ax4.grid(True, alpha=0.3)
    fig4.tight_layout()
    out4 = visuals_dir / f"solar_resonance_dphi_spectrum_v1_8_{ts_tag}.png"
    fig4.savefig(out4)
    plt.close(fig4)
    saved.append(str(out4))

    # 5) Harmonic Seal v2.1 (oracle triad + temporal texture)
    C = state.get("C", 0.0)
    ...
    texture = state.get("Temporal", {}).get("texture_entropy", 0.0)
    ...
    # Texture ring (outer subtle ring encoding temporal complexity)
    r_tex = max(0.0, min(1.0, texture))
    rT_list = [r_tex for _ in th]
    ax5.plot(th, rT_list, linestyle="--")

    ax5.set_xticks(angles)
    ax5.set_xticklabels(["E", "I", "C"])
    ax5.set_yticks([])
    ax5.set_title(f"Harmonic Seal v1.8\nband={band} • alert={alert}", va="bottom")
    fig5.tight_layout()
    out5 = visuals_dir / f"solar_resonance_harmonic_seal_v1_8_{ts_tag}.png"
    fig5.savefig(out5)
    plt.close(fig5)
    saved.append(str(out5))

    # 6) Solar QIM lattice (ΔΦ → 2D resonance map, fine)
    L = len(dphi_norm)
    ...
    ax6.set_xticks([])
    ax6.set_yticks([])
    ax6.set_title("Solar QIM Lattice v1.8 (fine)")
    fig6.colorbar(im3, ax=ax6, fraction=0.046, pad=0.04)
    fig6.tight_layout()
    out6 = visuals_dir / f"solar_resonance_qim_lattice_fine_v1_8_{ts_tag}.png"
    fig6.savefig(out6)
    plt.close(fig6)
    saved.append(str(out6))

    # 7) Multi-scale Solar QIM panel (coarse vs fine)
    # coarse: downsample by factor 2 before lattice
    step = 2
    dphi_ds = dphi_norm[::step] if len(dphi_norm) > 1 else dphi_norm
    Lc = len(dphi_ds)
    side_c = int(math.sqrt(Lc))
    if side_c < 1:
        side_c = 1
    Lc2 = side_c * side_c
    vals_c = dphi_ds[:Lc2]
    if len(vals_c) < Lc2:
        vals_c = vals_c + [0.0] * (Lc2 - len(vals_c))
    lattice_c = np.array(vals_c, dtype=float).reshape(side_c, side_c)

    fig7, (ax7a, ax7b) = plt.subplots(1, 2, figsize=(6, 3))
    im7a = ax7a.imshow(lattice_c, aspect="equal", interpolation="nearest")
    ax7a.set_xticks([])
    ax7a.set_yticks([])
    ax7a.set_title("Coarse QIM")

    im7b = ax7b.imshow(lattice, aspect="equal", interpolation="nearest")
    ax7b.set_xticks([])
    ax7b.set_yticks([])
    ax7b.set_title("Fine QIM")

    fig7.colorbar(im7b, ax=[ax7a, ax7b], fraction=0.046, pad=0.04)
    fig7.suptitle("Solar QIM Multi-Scale v1.8")
    fig7.tight_layout()
    out7 = visuals_dir / f"solar_resonance_qim_multiscale_v1_8_{ts_tag}.png"
    fig7.savefig(out7)
    plt.close(fig7)
    saved.append(str(out7))
...
def main():
    ...
    out_path = state_dir / f"solar_resonance_state_v1_8_{ts_safe}.json"
    ...
    ledger_entry = {
        ...
        "Temporal": state.get("Temporal", None),
        "visuals": visuals,
        "H7": H7,
        "node": "solar_resonance_v1_8",
    }
