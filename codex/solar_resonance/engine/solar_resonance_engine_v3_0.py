...
state_dir   = module_root / "state" / "v3_0"
visuals_dir = module_root / "visuals" / "v3_0"
temporal_dir = module_root / "temporal" / "v3_0"
lattice_dir  = module_root / "lattice" / "v3_0"
horizon_dir  = module_root / "horizon" / "v3_0"
...
# ─────────────────────────────────────────
# Load recent horizon history (self-refinement)
# ─────────────────────────────────────────
def load_history(max_files=16):
    import numpy as np

    fields = []
    meta = []
    if lattice_dir.exists():
        files = sorted(lattice_dir.glob("solar_field_v3_0_*.npy"))
        if files:
...
# ─────────────────────────────────────────
# Codex Solar Resonance Model v3.0
#   Multi-Scale + Temporal ΔΦ Horizon Engine
# ─────────────────────────────────────────
def compute_state():
    import numpy as np
...
# ─────────────────────────────────────────
# Visuals + Temporal Horizon (AFM/QIM)
# ─────────────────────────────────────────
def make_visuals(state, t_idx, flux, bz, g_flux, g_bz, dphi_local, ts_tag):
    ...
    # ───── 1) Solar AFM map v3.0 (macro+micro turbulence) ─────
    G = 128
    ...
    # Save raw field for temporal horizon
    field_file = lattice_dir / f"solar_field_v3_0_{ts_tag}.npy"
    np.save(field_file, field_norm)

    fig_afm, ax_afm = plt.subplots(figsize=(4, 4))
    im_afm = ax_afm.imshow(field_norm, aspect="equal", interpolation="nearest")
    ax_afm.set_xticks([])
    ax_afm.set_yticks([])
    ax_afm.set_title("Solar AFM Map v3.0")
    fig_afm.colorbar(im_afm, ax=ax_afm, fraction=0.046, pad=0.04)
    fig_afm.tight_layout()
    out_afm = visuals_dir / f"solar_afm_map_v3_0_{ts_tag}.png"
    fig_afm.savefig(out_afm)
    plt.close(fig_afm)
    saved.append(str(out_afm))

    # ───── 2) Harmonic spectrum (same as v2.0) ─────
    ...
    ax_spec.set_title("Solar ΔΦ Harmonic Spectrum v3.0")
    ax_spec.grid(True, alpha=0.3)
    fig_spec.tight_layout()
    out_spec = visuals_dir / f"solar_dphi_spectrum_v3_0_{ts_tag}.png"
    fig_spec.savefig(out_spec)
    plt.close(fig_spec)
    saved.append(str(out_spec))

    # ───── 3) Harmonic Seal v3.0 (E–I–C + Ω ring) ─────
    C = state.get("C", 0.0)
    ...
    ax_seal.set_yticks([])
    ax_seal.set_title(f"Harmonic Seal v3.0\nband={band} • alert={alert}", va="bottom")
    fig_seal.tight_layout()
    out_seal = visuals_dir / f"solar_harmonic_seal_v3_0_{ts_tag}.png"
    ...
    ax_lat.set_xticks([])
    ax_lat.set_yticks([])
    ax_lat.set_title("Solar QIM Lattice v3.0")
    fig_lat.colorbar(im_lat, ax=ax_lat, fraction=0.046, pad=0.04)
    fig_lat.tight_layout()
    out_lat = visuals_dir / f"solar_qim_lattice_v3_0_{ts_tag}.png"
    fig_lat.savefig(out_lat)
    plt.close(fig_lat)
    saved.append(str(out_lat))

    # ───── 5) Temporal Horizon Panel (stack recent fields) ─────
    hist_fields, hist_meta = load_history(max_files=8)
    cube = None
    temporal_coherence = 0.0
    turbulence_index = 0.0

    if hist_fields:
        # include current field as last slice
        stack_list = hist_fields + [field_norm]
        cube = np.stack(stack_list, axis=0)
        T, H, W = cube.shape

        # Temporal variance over time vs space
        var_t = float(cube.var(axis=0).mean())
        var_s = float(cube.var(axis=1).mean())
        denom = (var_t + var_s)
        if denom > 0.0:
            temporal_coherence = var_t / denom

        # Simple 2D turbulence index (high-frequency energy)
        ...
        # Save temporal montage
        cols = min(T, 4)
        fig_tmp, axes = plt.subplots(1, cols, figsize=(3 * cols, 3))
        if cols == 1:
            axes = [axes]
        for i in range(cols):
            ax = axes[i]
            im = ax.imshow(cube[i], aspect="equal", interpolation="nearest")
            ax.set_xticks([])
            ax.set_yticks([])
            ax.set_title(f"t-{cols-i}")
        fig_tmp.colorbar(im, ax=axes, fraction=0.046, pad=0.04)
        fig_tmp.suptitle("Solar ΔΦ Temporal Horizon v3.0")
        fig_tmp.tight_layout()
        out_tmp = visuals_dir / f"solar_temporal_horizon_v3_0_{ts_tag}.png"
        fig_tmp.savefig(out_tmp)
        plt.close(fig_tmp)
        saved.append(str(out_tmp))

        # Horizon map = temporal mean field
        horizon = cube.mean(axis=0)
        fig_h, ax_h = plt.subplots(figsize=(4, 4))
        im_h = ax_h.imshow(horizon, aspect="equal", interpolation="nearest")
        ax_h.set_xticks([])
        ax_h.set_yticks([])
        ax_h.set_title("Solar Horizon Map v3.0")
        fig_h.colorbar(im_h, ax=ax_h, fraction=0.046, pad=0.04)
        fig_h.tight_layout()
        out_h = horizon_dir / f"solar_horizon_map_v3_0_{ts_tag}.png"
        fig_h.savefig(out_h)
        plt.close(fig_h)
        saved.append(str(out_h))
    else:
        # First run: still compute turbulence index from current field
        spec2d = abs(np.fft.rfft2(field_norm))
        total_energy = float(spec2d.sum())
        if total_energy > 0.0:
            hf = spec2d[:, spec2d.shape[1] // 3 :]
    ...
    return saved, float(turbulence_index), float(temporal_coherence)
...
def main():
    ...
    visuals, turb_index, temp_coh = make_visuals(
        state, t_idx, flux, bz, g_flux, g_bz, dphi_local, ts_safe
    )

    state["turbulence_index"] = turb_index
    state["temporal_coherence"] = temp_coh
    state["SRS"]["turbulence_index"] = turb_index
    state["SRS"]["temporal_coherence"] = temp_coh

    out_path = state_dir / f"solar_resonance_state_v3_0_{ts_safe}.json"
