...
state_dir   = module_root / "state" / "v1_6"
visuals_dir = module_root / "visuals" / "v1_6"
...
# ─────────────────────────────────────────
# Codex Solar Resonance Model v1.6
#   Harmonic Oracle: C + spectral ΔΦ + drift
# ─────────────────────────────────────────
def compute_state():
    import numpy as np
    ...
    # Combined coherence score (as in v1.5)
    coherence_score = 0.0
    try:
    ...
    trend_score = 0.5
    try:
        # clamp dC/0.05 into [-1,1] then map to [0,1]
        z = dC / 0.05
        if z > 1.0:
            z = 1.0
        if z < -1.0:
...
# ─────────────────────────────────────────
# Visuals (Hybrid QIM + Harmonic Seal v2)
# ─────────────────────────────────────────
def make_visuals(state, t_idx, flux, bz, g_flux, g_bz, dphi_local, ts_tag):
    ...
    fig1.tight_layout()
    out1 = visuals_dir / f"solar_resonance_flux_bz_v1_6_{ts_tag}.png"
    fig1.savefig(out1)
    plt.close(fig1)
    ...
    fig2.tight_layout()
    out2 = visuals_dir / f"solar_resonance_grad_heatmap_v1_6_{ts_tag}.png"
    fig2.savefig(out2)
    plt.close(fig2)
    ...
    fig3.tight_layout()
    out3 = visuals_dir / f"solar_resonance_dphi_strip_v1_6_{ts_tag}.png"
    fig3.savefig(out3)
    plt.close(fig3)
    ...
    ax4.set_title("Solar ΔΦ Harmonic Spectrum v1.6")
    ax4.grid(True, alpha=0.3)
    fig4.tight_layout()
    out4 = visuals_dir / f"solar_resonance_dphi_spectrum_v1_6_{ts_tag}.png"
    fig4.savefig(out4)
    plt.close(fig4)
    saved.append(str(out4))

    # 5) Harmonic Seal v2 (polar triad + oracle)
    C = state.get("C", 0.0)
    ...
    ax5.set_yticks([])
    ax5.set_title(f"Harmonic Seal v1.6\nband={band} • alert={alert}", va="bottom")
    fig5.tight_layout()
    out5 = visuals_dir / f"solar_resonance_harmonic_seal_v1_6_{ts_tag}.png"
    fig5.savefig(out5)
    plt.close(fig5)
...
def main():
    ...
    out_path = state_dir / f"solar_resonance_state_v1_6_{ts_safe}.json"
