...
- **What:** `codex/config` is a Codex runtime domain folder that stores module logic, state, logs, or related artifacts for the **config** subsystem.
- **Why:** This folder exists to isolate `config` responsibilities, reduce coupling with other domains, and make evolution/versioning safer over time.
...
- **Where:** Path: `codex/config` (within the core Codex runtime tree).
