...
function TryJson($p){ if (Test-Path $p){ try { Get-Content -Raw -Encoding UTF8 $p | ConvertFrom-Json } catch { $null } } else { $null } }
        ...
        "reflection" {
          ...
          try {
            $conv = [ordered]@{
              ...
              id        = $rid
              agent     = (""+$req.agent)
              message   = if ($req.payload -and $req.payload.message) { (""+$req.payload.message) } else { "" }
