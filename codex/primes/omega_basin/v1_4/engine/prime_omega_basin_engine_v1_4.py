#!/usr/bin/env python3
# ╔══════════════════════════════════════════════════════════════╗
# ║  PRIME Ω-BASIN v1.4 — REFERENCE-ANCHORED NOISE SWEEP          ║
# ║  v1.4: one base-noise field scaled across sweep (comparability)║
# ╚══════════════════════════════════════════════════════════════╝
...
def main(root, state_d, vis_d, ledger_d, logs_d, limit, T, noise_levels_csv):
    ...
    try:
        ...
        # enforce sorted + unique for stability
        noise_levels = sorted(list(dict.fromkeys(noise_levels)))

        log("PRIME OMEGA-BASIN v1.4 starting...")
        ...
        #    build one fixed base noise field ~ N(0,1), then scale by sigma per noise_level
        ...
        # also capture a representative noisy Ω curve at max noise
        max_noise_level = max(noise_levels) if noise_levels else 0.0
        omega_t_max = None
        ...
        if omega_t_max is not None:
            ...
            plt.plot(omega_t_max, label=f"Omega(t) noisy (noise={max_noise_level:g})", linestyle="--")
            plt.legend()
        ...
        state = {
            ...
            "codex": {
                ...
                "law": "Omega = 1/(1+|dPhi|)",
                "note_v1_4": "REFERENCE-ANCHORED SWEEP: one base noise field scaled across sweep."
            }
        }
        ...
        log("PRIME OMEGA-BASIN v1.4 complete.")
        return 0
...
if __name__ == "__main__":
    # Usage: engine ROOT STATE VIS LEDGER LOGS LIMIT T NOISE_SWEEP_CSV
    if len(sys.argv) < 9:
        print("Usage: engine ROOT STATE VIS LEDGER LOGS LIMIT T NOISE_SWEEP_CSV", file=sys.stderr)
        sys.exit(1)

    _, root, state, vis, led, logs, lim, T, noise_csv = sys.argv[:9]
